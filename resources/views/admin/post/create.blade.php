            ...
            <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <textarea name="content" id="content" rows="10" class="form-input-premium" placeholder="Escreva seu artigo aqui..." required>{{ old('content') }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-[10px] uppercase font-bold mt-2 ml-1 tracking-widest">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="content_en" id="content_en" rows="10" class="form-input-premium" placeholder="Write your article in English...">{{ old('content_en') }}</textarea>
                        @error('content_en')
                            <p class="text-red-500 text-[10px] uppercase font-bold mt-2 ml-1 tracking-widest">{{ $message }}</p>
                        @enderror
                    </div>
