        ...
        <div class="glass-card p-10">
            <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <label class="form-label-premium">Capa Atual</label>
                        @if($post->image)
                            <div class="mb-4">
                                <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" class="h-32 w-full object-cover rounded-xl border border-white/10">
                            </div>
                        @endif
                        
                        <label for="image" class="form-label-premium mt-4">Alterar Capa</label>
                        <div class="relative group/file">
                            ...
                            <div class="w-full bg-white/5 border border-white/5 border-dashed rounded-xl py-6 px-6 text-center group-hover/file:border-gold-accent/30 transition duration-300">
                                ...
                                <p class="text-[10px] text-gray-400">Selecionar Nova Capa</p>
                            </div>
                        </div>
                        @error('image')
                            <p class="text-red-500 text-[10px] uppercase font-bold mt-2 ml-1 tracking-widest">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="published_at" id="published_at" class="form-input-premium py-3" value="{{ old('published_at', $post->published_at ? \Carbon\Carbon::parse($post->published_at)->format('Y-m-d\TH:i') : '') }}">
                        @error('published_at')
                            <p class="text-red-500 text-[10px] uppercase font-bold mt-2 ml-1 tracking-widest">{{ $message }}</p>
                        @enderror
                    </div>
