<section id="resume" class="bg-deep-black py-24 md:py-32 px-4">
    <div class="container mx-auto max-w-7xl">
        <div class="reveal text-center mb-16 md:mb-24">
            <h2 class="text-gold-accent text-sm font-semibold tracking-[0.2em] uppercase mb-4">Experiência</h2>
            <p class="text-4xl md:text-5xl font-bold text-white font-outfit">Carreira & Formação</p>
                ...
                <div class="space-y-8 border-l border-white/10 ml-4 pl-8">
                    @forelse ($experiences as $experience)
                        <div class="relative">
                            ...
                            <div class="glass-card p-6">
                                <span class="text-gold-accent text-[10px] font-bold uppercase tracking-widest block mb-2">
                                    {{ \Carbon\Carbon::createFromDate($experience->year_start, $experience->start_month, 1)->locale('pt_BR')->translatedFormat('M/Y') }} - {{ $experience->year_end ? \Carbon\Carbon::createFromDate($experience->year_end, $experience->end_month, 1)->locale('pt_BR')->translatedFormat('M/Y') : 'Atual' }}
                                </span>
                                <h4 class="text-xl font-bold text-white mb-2">{{ $experience->position }}</h4>
                                <p class="text-gray-400 font-medium text-sm mb-4">{{ $experience->company }}</p>
                                @if($experience->description)
                                    <p class="text-gray-500 text-sm font-light leading-relaxed">{{ $experience->description }}</p>
                                @endif
                            </div>
                        </div>
                    @empty
