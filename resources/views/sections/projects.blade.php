<section id="projects" class="bg-deep-black py-24 md:py-32 px-4">
    <div class="container mx-auto max-w-7xl">
        <div class="reveal text-center mb-16 md:mb-24">
            <h2 class="text-gold-accent text-sm font-semibold tracking-[0.2em] uppercase mb-4">{{ __('messages.projects.title') }}</h2>
            <p class="text-4xl md:text-5xl font-bold text-white font-outfit">{{ __('messages.projects.subtitle') }}</p>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            @forelse ($projects as $project)
                <div class="glass-card overflow-hidden group">
                    <div class="relative overflow-hidden aspect-video">
                        @if($project->image)
                            <img src="{{ asset('storage/' . $project->image) }}" alt="{{ $project->t('title') }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-700">
                        @else
                    ...
                    <div class="p-8">
                        <h3 class="text-2xl font-bold text-white mb-3 group-hover:text-gold-accent transition-colors">{{ $project->t('title') }}</h3>
                        <p class="text-gray-400 text-sm mb-6 leading-relaxed line-clamp-3 font-light">{{ $project->t('description') }}</p>
            ...
            @empty
                <div class="col-span-full text-center py-20 glass-card">
                    <p class="text-gray-500 italic">{{ __('messages.projects.empty') }}</p>
                </div>
            @endforelse
        </div>
    </div>
